                        ...
                        map< int, map< string, string > >& outObjects )
{
    ...
    {
        loggers.write( "[InputLoader] WARN: Unknown IMPORT INPUT_MODE '" +
                               ...
                               map< int, map< string, string > >& outObjects )
{
    ...
    {
        loggers.writefaillog( "[InputLoader] ERROR: CSV mode selected but [DELETE] CSV_FILE is empty" );
        return false;
    }
    ...
    if( !fpIn.is_open() )
    {
        loggers.writefaillog( "[InputLoader] ERROR: Cannot open input file: " + inSettings.csvFile );
        return false;
    }
    ...
    loggers.write( "[InputLoader] CSV mode: loaded " + to_string( ( int )outObjects.size() ) +
                              ...
                              map< int, map< string, string > >& outObjects )
{
    ...
    {
        loggers.writefaillog( "[InputLoader] ERROR: DB mode selected but [DELETE] DB_QUERY is empty" );
        return false;
    }
    ...
    {
        loggers.writefaillog( "[InputLoader] ERROR: DB connect failed: " + oDb.lastError() );
        return false;
    }
    ...
    {
        loggers.writefaillog( "[InputLoader] ERROR: DB query failed: " + oDb.lastError() );
        oDb.disconnect();
        return false;
    }
    ...
    loggers.write( "[InputLoader] DB mode: loaded " + to_string( ( int )outObjects.size() ) +
                  " object(s) from query" );
    return !outObjects.empty();
}
                                    ...
                                    map< int, map< string, string > >& outObjects )
{
    ...
    {
        loggers.writefaillog( "[InputLoader] ERROR: CSV_DB mode selected but [DELETE] CSV_DB_QUERY is empty" );
        return false;
    }
    ...
    for( size_t r = 0; r < vecCsvRows.size(); r++ )
    {
        ...
        {
            loggers.write( "[InputLoader] WARN: No DB row for ITEM_ID '" + sId +
                          "' and CSV_DB_FALLBACK_TO_CSV is disabled - skipped" );
        }
    }
    oDb.disconnect();

    loggers.write( "[InputLoader] CSV_DB mode: loaded " + to_string( iLoaded ) + " DB row(s)" +
                               ...
                               vector< vector< string > >& outRows )
{
    if( inSettings.csvFile.empty() )
    {
        loggers.writefaillog( "[InputLoader] ERROR: CSV_DB mode selected but [DELETE] CSV_FILE is empty" );
        return false;
    }
    ...
    if( !fpIn.is_open() )
    {
        loggers.writefaillog( "[InputLoader] ERROR: Cannot open CSV file: " + inSettings.csvFile );
        return false;
    }
    ...
    {
        loggers.writefaillog( "[InputLoader] ERROR: CSV file has no header row: " + inSettings.csvFile );
        return false;
    }
