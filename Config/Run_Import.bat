...
rem =============================================================================
rem  Run_Import.bat - runs ClassificationImport.exe
rem
...
rem
rem  The exe itself reads [IMPORT] INPUT_MODE, CSV_FILE, DB_QUERY,
rem  CSV_DB_QUERY directly from the same config file.
rem =============================================================================
...
set "EXE=%ROOTDIR%\x64\Release\ClassificationImport.exe"
if not exist "%EXE%" (
    echo ERROR : %EXE% not found.
    echo         Build it first with Build_All.bat IMPORT or from Visual Studio.
    goto :end
)

echo ============================================================
echo  ClassificationImport
...
set "RC=%errorlevel%"
echo.
echo ClassificationImport finished with exit code %RC%.
