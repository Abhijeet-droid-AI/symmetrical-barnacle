...
bool DBConnector::connect( const string& inConnStr )
{
    ...
    if ( !SQL_SUCCEEDED( nRet ) )
    {
        ...
        logger.writebothlog( "DB ERROR : SQLAllocHandle(ENV) failed" );
        return false;
    }
    ...
    if ( !SQL_SUCCEEDED( nRet ) )
    {
        ...
        logger.writebothlog( "DB ERROR : SQLAllocHandle(DBC) failed" );
                                ...
                                vector< vector< string > >& outRows )
{
    ...
    if ( !m_bConnected )
    {
        logger.writebothlog( "DB ERROR : executeQuery called without a connection" );
        return false;
    }
    ...
    /* read-only safety guard - this utility never modifies the database */
    {
        ...
        if ( sHead != "SELECT" )
        {
            ...
            logger.writebothlog( "DB ERROR : only SELECT statements are allowed (query: "
                                 + inQuery + ")" );
            return false;
        }
    }
    ...
    if ( !SQL_SUCCEEDED( nRet ) )
    {
        ...
        logger.writebothlog( "DB ERROR : SQLAllocHandle(STMT) failed" );
        return false;
    }
    ...
    if ( nColumns < 1 )
    {
        logger.writebothlog( "DB ERROR : query returned no columns (query: "
                             + inQuery + ")" );
        SQLFreeHandle( SQL_HANDLE_STMT, hStmt );
        return false;
    }
    ...
    if ( outRows.empty() )
        logger.write( "DB INFO : query returned no rows (query: " + inQuery + ")" );
